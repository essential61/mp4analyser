...
def box_factory(fp, header):
    the_box = None
    if header.type == 'ftyp':
        the_box = FtypBox(fp, header)
    elif header.type == 'pdin':
        the_box = PdinBox(fp, header)
    elif header.type == 'moov':
        the_box = MoovBox(fp, header)
    elif header.type == 'mvhd':
        the_box = MvhdBox(fp, header)
    elif header.type == 'meta':
        the_box = MetaBox(fp, header)
    elif header.type == 'trak':
        the_box = TrakBox(fp, header)
    elif header.type == 'tkhd':
        the_box = TkhdBox(fp, header)
    elif header.type == 'tref':
        the_box = TrefBox(fp, header)
    elif header.type == 'trgr':
        the_box = TrgrBox(fp, header)
    elif header.type == 'edts':
        the_box = EdtsBox(fp, header)
    elif header.type == 'elst':
        the_box = ElstBox(fp, header)
    elif header.type == 'mdia':
        the_box = MdiaBox(fp, header)
    elif header.type == 'mdhd':
        the_box = MdhdBox(fp, header)
    elif header.type == 'hdlr':
        the_box = HdlrBox(fp, header)
    elif header.type == 'elng':
        the_box = ElngBox(fp, header)
    elif header.type == 'minf':
        the_box = MinfBox(fp, header)
    elif header.type == 'vmhd':
        the_box = VmhdBox(fp, header)
    elif header.type == 'smhd':
        the_box = SmhdBox(fp, header)
    elif header.type == 'hmhd':
        the_box = HmhdBox(fp, header)
#    elif header.type == 'sthd':
#        the_box = SthdBox(fp, header)
    elif header.type == 'nmhd':
        the_box = NmhdBox(fp, header)
    elif header.type == 'dinf':
        the_box = DinfBox(fp, header)
    elif header.type == 'dref':
        the_box = DrefBox(fp, header)
    elif header.type == 'url ':
        the_box = Url_Box(fp, header)
    elif header.type == 'urn ':
        the_box = Urn_Box(fp, header)
    elif header.type == 'stbl':
        the_box = StblBox(fp, header)
    elif header.type == 'stsd':
        the_box = StsdBox(fp, header)
    elif header.type == 'stts':
        the_box = SttsBox(fp, header)
    elif header.type == 'ctts':
        the_box = CttsBox(fp, header)
    elif header.type == 'cslg':
        the_box = CslgBox(fp, header)
    elif header.type == 'stsc':
        the_box = StscBox(fp, header)
    elif header.type == 'stsz':
        the_box = StszBox(fp, header)
    elif header.type == 'stz2':
        the_box = Stz2Box(fp, header)
    elif header.type == 'stco':
        the_box = StcoBox(fp, header)
    elif header.type == 'co64':
        the_box = VmhdBox(fp, header)
    elif header.type == 'stss':
        the_box = StssBox(fp, header)
...
#        the_box = SgpdBox(fp, header)
    elif header.type == 'subs':
        the_box = SubsBox(fp, header)
#    elif header.type == 'saiz':
#        the_box = SaizBox(fp, header)
#    elif header.type == 'saio':
#        the_box = SaioBox(fp, header)
    elif header.type == 'udta':
        the_box = UdtaBox(fp, header)
#    elif header.type == 'mvex':
#        the_box = MvexBox(fp, header)
#    elif header.type == 'mehd':
#        the_box = MehdBox(fp, header)
#    elif header.type == 'trex':
#        the_box = TrexBox(fp, header)
#    elif header.type == 'leva':
#        the_box = LevaBox(fp, header)
    elif header.type == 'moof':
        the_box = MoofBox(fp, header)
    elif header.type == 'mfhd':
        the_box = MfhdBox(fp, header)
    elif header.type == 'traf':
        the_box = TrafBox(fp, header)
    elif header.type == 'tfhd':
        the_box = TfhdBox(fp, header)
    elif header.type == 'trun':
        the_box = TrunBox(fp, header)
    elif header.type == 'tfdt':
        the_box = TfdtBox(fp, header)
#    elif header.type == 'mfra':
#        the_box = MfraBox(fp, header)
#    elif header.type == 'tfra':
#        the_box = TfraBox(fp, header)
#    elif header.type == 'mfro':
#        the_box = MfroBox(fp, header)
    elif header.type == 'mdat':
        the_box = MdatBox(fp, header)
    elif header.type == 'free':
        the_box = FreeBox(fp, header)
    elif header.type == 'skip':
        the_box = SkipBox(fp, header)
#    elif header.type == 'cprt':
#        the_box = CprtBox(fp, header)
#    elif header.type == 'tsel':
#        the_box = TselBox(fp, header)
#    elif header.type == 'strk':
#        the_box = StrkBox(fp, header)
#    elif header.type == 'stri':
#        the_box = StriBox(fp, header)
#    elif header.type == 'strd':
#       the_box = StrdBox(fp, header)
#    elif header.type == 'iloc':
#        the_box = IlocBox(fp, header)
#    elif header.type == 'ipro':
#        the_box = IproBox(fp, header)
#    elif header.type == 'sinf':
#        the_box = SinfBox(fp, header)
#    elif header.type == 'frma':
#        the_box = FrmaBox(fp, header)
#    elif header.type == 'schm:
#       the_box = SchmBox(fp, header)
#    elif header.type == 'iinf':
#        the_box = IinfBox(fp, header)
#    elif header.type == 'xml ':
#        the_box = Xml_Box(fp, header)
#    elif header.type == 'bxml':
#        the_box = BxmlBox(fp, header)
#    elif header.type == 'pitm':
#        the_box = PitmBox(fp, header)
#    elif header.type == 'fiin':
#       the_box = FiinBox(fp, header)
#    elif header.type == 'paen':
#        the_box = PaenBox(fp, header)
#    elif header.type == 'fire':
#        the_box = FireBox(fp, header)
#    elif header.type == 'fpar':
#        the_box = FparBox(fp, header)
#    elif header.type == 'fecr':
#        the_box = FecrBox(fp, header)
#    elif header.type == 'segr:
#       the_box = SegrBox(fp, header)
...
#        the_box = MereBox(fp, header)
    elif header.type == 'styp':
        the_box = StypBox(fp, header)
    elif header.type == 'sidx':
        the_box = SidxBox(fp, header)
    elif header.type == 'ssix':
        the_box = SsixBox(fp, header)
    elif header.type == 'prft':
        the_box = PrftBox(fp, header)
    else:
        return mp4.non_iso.box_factory_non_iso(fp, header)
    return the_box
# Box classes
...
class MdatBox(Mp4Box):
    ...
    def __init__(self, fp, header):
        super().__init__(header)
        self.start_of_box = fp.tell()
        self.box_info['message'] = 'Haven\'t decided what to display here'
        ...
        for i in range(self.box_info['entry_count']):
            ...
            if subsample_count > 0:
                ...
                for j in range(subsample_count):
                    ...
                    subsample_list.append({'subsample_size': subsample_size, 'subsample_priority': subsample_priority,
                    'discardable': discardable, 'codec_specific_parameters': codec_specific_parameters})
                self.box_info['entry_list'].append({'sample_delta': sample_delta, 'subsample_count': subsample_count,
...
class SbgpBox(Mp4FullBox):
    ...
    def __init__(self, fp, header):
        ...
        self.box_info['entry_list'] = []
        for i in range(self.box_info['entry_count']):
            self.box_info['entry_list'].append({'sample_count': read_u32(fp), 'group_description_index': read_u32(fp)})
        fp.seek(self.start_of_box)
...
class SaizBox(Mp4FullBox):
    ...
    def __init__(self, fp, header):
        ...
        self.box_info['sample_count'] = read_u32(fp)
        # spec is unclear, is it an array?
        fp.seek(self.start_of_box)
...
class SidxBox(Mp4FullBox):
    ...
    def __init__(self, fp, header):
        ...
        for i in range(self.box_info['reference_count']):
            ...
            st_sz = read_u32(fp)
            self.box_info['reference_list'].append({'reference_type': rt_sz // 2147483648, 'reference_size': rt_sz % 2147483648,
                                   'subsegment_duration': subsegment_dur, 'starts_with_sap': st_sz // 2147483648,
                                   'SAP_type': (st_sz % 2147483648) // 268435456, 'SAP_delta_time': st_sz // 268435456})
        fp.seek(self.start_of_box)
...
class SsixBox(Mp4FullBox):
    ...
    def __init__(self, fp, header):
        ...
        for i in range(self.box_info['subsegment_count']):
            ...
            range_list = []
            for i in range(self.box_info['range_count']):
                l_r = read_u32(fp)
